...
async fn process_file<B: RunBrowser>(
    ...
    for directive in directives.iter().filter(|d| {
        ...
        let today = today_iso();
        let environment = context::build_environment(root, path, &today);
        process_directive(
            client,
            &directive.prompt,
            &document,
...
fn today_iso() -> String {
    ...
    // Civil date calculation from days since epoch
...
async fn process_directive<B: RunBrowser>(
    ...
    browser: Option<&B>,
    path: &Path,
    environment: &llm::EnvironmentContext,
) {
    let system_prompt = llm::build_system_prompt(document, browser.is_some(), environment);
...
fn execute_tool<B: RunBrowser>(
    ...
    match call.tool.as_str() {
        ...
        "delete" => tools::delete::DeleteTool::new(root.to_path_buf()).execute(&call.input),
...
";
    ...
    async fn process_directive__should_stop_when_no_response_block() {
        ...
        process_directive(
            ...
            NO_BROWSER,
            &path,
            &llm::EnvironmentContext::default(),
        )
        .await;
